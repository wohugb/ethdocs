...
case of executing the `geth account new` command in a terminal.
...
1.  [Download the latest version of the wallet
    app](https://github.com/ethereum/mist/releases) for your operating
...
![downloading-mist](img/51Downloading.png){.align-center width="436px"
height="352px"}

3.  Wait for the blockchain to fully sync, then follow the instructions
...
![opening-screen](img/51OpeningScreen.png){.align-center width="512px"
height="469px"}
...
create a wallet, run `ethkey` with the `createwallet` command:
...
To create a key, we use the `new` command. To use it we must pass a name
...
![presale-import](img/51PresaleImportInstall.png){.align-center
...
in the app's menu bar and then selecting `Import Pre-sale Accounts`.
